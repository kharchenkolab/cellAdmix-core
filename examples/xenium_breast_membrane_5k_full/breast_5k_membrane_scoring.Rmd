...
mode: the over-removal guardrails fire, reporting that endothelial and
fibroblast cells lose essentially *all* of their molecules:
...
The mechanism: the rank-8 `ls_nmf` fit collapses to a few active factors,
none of them native to the small endothelial and fibroblast populations.
...
no native factor to hold the genuinely endothelial molecules, the union of
the rules covers the type's entire factor spectrum, and the correction
erases the cell type instead of cleaning it.
...
```{r bridge-vs-membrane-pairs, fig.width=12.5, fig.height=5.6, warning=FALSE}
...
correction retains per cell type (left) — the bridge correction's erasure
of the stain-weak types — while the overall admixture burden removed
(right) stays similar:

```{r bridge-vs-membrane-retention, fig.width=11.0, fig.height=4.2}
retention <- rbind(
