...
detected pairs. The recommended configuration initializes the model’s
expression programs from an NMF fit, which is the only role the fit
plays here; `init = "clusters"` derives the programs from the data
directly, without any NMF:
