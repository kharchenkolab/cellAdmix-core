...
of this page runs the same workflow in a handful of calls).
...
expression programs: the recommended configuration initializes them
from an NMF fit's factor-labeled molecules, which preserves the most
induced biology (`init = "clusters"` fits without any NMF, at nearly
identical removal but lower induced-gene retention on large panels).
The correction then derives from the fitted model:
...
```{r composition, fig.width=6, fig.height=3.8}
...
```{r composition-map, fig.width=8.5, fig.height=3.2}
...
cell-type contours. Here the molecules are colored by gene origin:
blue for the ductal marker genes (the cell's own identity), red for
the exocrine marker genes (the audit's admixture panel for this pair —
...
```{r anatomy-molecules, fig.width=11, fig.height=5.4}
...
The audit's standard per-pair diagnostic — the panel rate with exact
95% intervals, the zero-neighbor line, and the ambient reference —
verifies the pair directly, before and after the correction:

```{r pair-exposure, fig.width=6.5, fig.height=4.2}
...
```{r volcano, fig.width=6.8, fig.height=5}
...
```{r screen, fig.width=7, fig.height=5}
...
so the markers superimpose. The dashed curve is the largest excess
still consistent with proportional transfer, allowing counting noise
and a 15% profile error.
...
```{r gene-contrast, fig.width=8.5, fig.height=3.8}
...
```{r cftr-map, fig.width=8.5, fig.height=3.2}
...
```{r state-umap, fig.width=11, fig.height=4.8}
