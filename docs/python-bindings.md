...
induced genes — exposure-linked excess disproportionate to the source
expression profile indicates a transcriptional response to proximity
...
excluded, replaced by the next-ranked source-specific genes, and listed
with their measured disproportionality in `markers()["induced"]` and
`markers()["induced_stats"]`. The audit measures every ordered cell-type pair
